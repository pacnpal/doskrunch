...
//!
//! `#[ignore]`-gated: needs `dosbox-x` installed locally, takes minutes
//! to run, and writes a tracked file. Run with
...
#[test]
#[ignore = "needs dosbox-x; regenerates tests/benchmarks/results.md"]
fn benchmark_tier_decompression() {
    ...
    for (tier, cputype) in tiers {
        ...
        for run in 0..RUNS_PER_TIER {
            ...
            let dosbox_status = match wait_with_timeout(&mut dosbox, DOSBOX_TIMEOUT) {
                Ok(s) => s,
                Err(_) => panic!("dosbox-x timeout for tier {tier} run {run}"),
            };
            assert!(
    ...
    // Report-only: the host harness prints the ratios for inspection but
    // does not assert against them. PLAN.md's 2–4× / 5–10× targets
    // describe real-hardware behaviour; DOSBox-X's variable per-cputype
    // emulation cost means the wall-clock numbers from this harness can
    // and do diverge. A failing ratio assertion here would flake CI on
    // unrelated host-side scheduling noise — see the caveats block at
...
fn write_results_markdown(root: &Path, results: &[TierResult], payload: &[u8]) {
    ...
    md.push_str("Measurement: end-to-end SFX wall-clock under headless DOSBox-X with `cycles=auto`, \
        min across 3 runs per tier. Run with:\n\n");
    ...
    md.push_str(
         ...
         * PLAN.md §10's 2–4× / 5–10× targets describe real-hardware behaviour. If the table \
           above misses those, the next step is real-iron measurement (86Box or a real \
           486/Pentium box), *not* re-tuning the asm port.\n\
         * The benchmark is `#[ignore]`-gated and runs on demand; CI doesn't regenerate this \
           file. Commit a refreshed copy when the numbers move.\n",
    );
