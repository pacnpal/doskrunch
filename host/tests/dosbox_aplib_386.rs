//! Phase 3 verify gate: pack the fixture set with `--algo aplib --target 386`,
//! run the SFX under headless DOSBox-X at `cputype=386`, and confirm
//! extracted files match the originals byte-for-byte.
//!
//! Parallel to `dosbox_aplib_8086.rs` — same fixture set, same timeout
//! pattern, same headless config. Differences: `--target 386` on pack
//! (selects `stubs/blobs/aplib_386.bin`, the wcc -3 + 32-bit-register
//! depacker stub) and `cputype=386` on the emulator (pins the CPU to
//! the lowest tier that supports operand-size-prefixed 32-bit ALU ops
//! in real mode). Catches the worst class of port bug: code that
//! assembles and links but emits 386-only instructions that wouldn't
//! actually be safe on the targeted real-mode CPU.
//!
...
fn extracts_aplib_fixtures_under_386_cputype() {
    let root = repo_root();
