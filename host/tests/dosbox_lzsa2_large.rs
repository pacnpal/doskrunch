//! Phase 6 multi-chunk LZSA2 correctness gate: pack a 500 KiB
//! synthetic mixed-content payload with `--algo lzsa2` at every
//! shipped tier, run under headless DOSBox-X at the matching
//! `cputype=`, and assert byte-identical extraction.
//!
//! Parallel to dosbox_aplib_large.rs and dosbox_lzma_large.rs. With
//! LZSA2_CHUNK_INPUT = 16 KiB a 500 KiB payload spans ~32 chunks per
//! file, so the stub's per-chunk decode loop and the LZSA2 ASM
//! depacker get exercised end-to-end on each emulated CPU. Catches
//! the class of bug the single-chunk per-tier gate misses: an
//! off-by-one between chunks would pass the small-fixture gate (each
//! fixture is < 16 KiB, one chunk) but corrupt the second chunk of
//! any real payload.
//!
...
/// Same mixed-content payload `dosbox_aplib_large.rs` and
/// `dosbox_lzma_large.rs` use (kept inline for the same reason
/// they keep it inline — this correctness gate shouldn't depend
/// on the benchmark harness).
fn synthesize_payload() -> Vec<u8> {
    let text = b"doskrunch phase 6 lzsa2 payload\n";
...
fn extracts_500kib_lzsa2_payload_across_all_tiers() {
    ...
    for (tier, cputype) in tiers {
        ...
        let extracted = locate_case_insensitive(rundir_path, "PAYLOAD.BIN")
            .unwrap_or_else(|| panic!("missing extracted PAYLOAD.BIN on lzsa2 tier {tier}"));
        let body = fs::read(&extracted)
            .unwrap_or_else(|e| panic!("read extracted PAYLOAD.BIN (lzsa2 tier {tier}): {e}"));
        assert_eq!(
            body.len(),
            payload.len(),
            "lzsa2 tier {tier}: extracted PAYLOAD.BIN length mismatch ({} vs {})",
            body.len(),
            payload.len()
        );
        assert_eq!(
            body, payload,
            "lzsa2 tier {tier}: extracted PAYLOAD.BIN body differs from source"
        );
    }
}
