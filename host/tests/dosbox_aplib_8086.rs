//! Phase 2 verify gate: pack the fixture set with `--algo aplib`, run the
//! SFX under headless DOSBox-X at `cputype=8086`, and confirm extracted
//! files match the originals byte-for-byte.
//!
//! Mirrors `dosbox_8086.rs` (the stored-path test) — same fixture set,
//! same timeout pattern, same headless config. The only difference is
//! the `--algo aplib` flag passed to `doskrunch pack` and the assertion
//! that the embedded 16-bit aPLib depacker (ported from apultra's
//! `asm/8088/aplib_8088_small.S`) round-trips the apultra-compressed
//! payload correctly on a real 8086 CPU.
//!
...
fn extracts_aplib_fixtures_under_8086_cputype() {
    let root = repo_root();
