...
- **MMX-vs-pentium aplib speed gate** (PLAN.md §10). aPLib's bit-at-a-time
  ...
  `tests/benchmarks/results.md` for the full rationale. The bench-only
  decode-timing harness (`make bench` blobs + `benchmark_tier_decompression`
  in `host/tests/benchmark_tiers.rs`) measures isolated decode time via the
  INT 1Ah BIOS tick counter (8086+) for local measurement.
