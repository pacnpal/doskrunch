...
Default invocation today (Phase 4): `doskrunch pack out.exe files-or-dirs...` → `--algo aplib --target 8086`. Directory inputs are walked recursively (symlinks skipped, whether named directly as an input or found during the walk); files extract flat at runtime regardless of the source tree's shape. The output path is excluded from the walk so `pack dir/OUT.EXE dir/` doesn't pack a previous SFX into the new one. The 8086 stub dispatches at runtime on the archive's algorithm byte, so `--algo stored` keeps working against the same blob. Phase 3 also ships `--target 386` (wcc -3 + 32-bit-register aPLib depacker from `aplib_depack_32.asm`) and `--target pentium` (wcc -5 + speed-optimized fast-variant depacker from `aplib_depack_p5.asm`); both blobs dispatch on the archive's algorithm byte the same way the 8086 blob does. `--chunk-size <bytes>` (Phase 4) overrides the default 16 KiB chunk; the ceiling is 16 KiB for aplib (stub BSS budget) and `u16::MAX` for stored. Today pack reads each input fully into memory before encoding, so the value controls archive layout, not host RAM.
...
Nine DOSBox-X correctness gates ship (six from Phase 3, three added in Phase 4):
