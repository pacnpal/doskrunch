...
- `dosbox_timestamps` (Phase 4, unix-only) — two cases. (1) A pinned source mtime (deliberately not "now", set via `filetime::set_file_mtime`) round-trips through pack → DOSBox-X → host `fs::metadata`; the archive's timestamp field is checked exactly, and the extracted file's mtime is decomposed via `libc::localtime_r` into LOCAL broken-down components which must equal the FAT components the host wrote exactly (DOS interprets FAT dates as local with no timezone concept, so the comparison is on (year, month, day, hour, min, sec÷2) — no slack). (2) A non-Jan-1 pre-1980 source mtime (1979-06-15 17:42:00 UTC) is fully clamped to the FAT epoch endpoint by `fat_time::unix_to_fat`; the extracted file's LOCAL components must equal (1980, 1, 1, 0, 0, 0) exactly — a year-only clamp regression would produce (1980, 6, 15, 17, 42, 0) and fail.
